...
public:
    ...
    Hotel(string nm, int nit = 0, int sin = 0, int dbl = 0, double pns = 0, double pnd = 0, bool prk = false, Board brd = noMeals)
         : name(nm), nights(nit), singles(sin), doubles(dbl), priceNightSingle(pns), priceNightDouble(pnd), parking(prk), board(brd) {}
    ~Hotel ()
    {
...
public:
    Publictransport () { }
    Publictransport (class Date dep, string cod, string fr, string t, double pos = 0, bool fc = false)
...
public:
    Flight () { }
    Flight (class Date dep, string c = "", string f = "", string t = "", double pos = 0, bool fc = false, bool tr = true)
